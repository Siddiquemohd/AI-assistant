...
class MockAiProvider(BaseAiProvider):
    ...
    def _process_nlu(self, history: List[PromptMessage]) -> str:
        ...
        if not user_messages:
            return "Hello! I am ISAI, your personal AI assistant equipped with 20 capability engines. How can I assist you today?"

        last_prompt = user_messages[-1].strip()
        text_lower = last_prompt.lower()

        # 1. System Health & Diagnostics Intent
        ...
        if any(w in text_lower for w in ["ocr", "extract text", "read document scan"]):
            return (
                ...
                f"Extracted Text: Personal Uncensored ISAI Model v2.0 - 20 Engines Active\n"
                f"Confidence: 99.8%\n"
                f"```"
            )

        # 4. Phone Call NLU Intent
        ...
        # 6. Application Launch NLU Intent
        ...
        # 8. Reminder Scheduling NLU Intent
        ...
        if any(w in text_lower for w in ["hi", "hello", "hey", "who are you", "what can you do"]):
            ...
            return (
                f"Hello! You said: '{last_prompt}'. I am **ISAI**, your 20-Engine Uncensored Personal AI Assistant running live on Render 24/7.{pref_note}"
            )

        # 11. Uncensored & Unfiltered Response
        return (
            f"Hello! I processed your message: **\"{last_prompt}\"**.\n\n"
            f"I am **ISAI**, your personal AI assistant running 100% uncensored without content filters. All 20 capability engines (Calls, Web Search, App Launch, Tasks, Neural Voice, Math, Vision Gen/Edit, Knowledge Graph, Emotion AI, Agent Planner, System Diagnostics) are live on Render 24/7."
        )
